...
void reset(status arg) {
  ...
  set_name("eye");
  set_alt_name("spell");
  set_short(B_BLUE_F+"floating eye"+NORM);
  set_long("\
...
void time_out() {
  if(master) {
    tell_object(master,"The scrying eye dispels.\n");
  }
...
void new_room(string str) {
  ...
  if(old_room != environment() ||
    str == "l" || str == "look") {
    if(environment()->query_no_scry()) {
      write(B_WHITE_F+"You are blinded as your scrying eye suddenly disintegrates in a flash of white!\n"+NORM);
      destruct(this_object());
      return; }
    write(
"#################################################################\n"+
...
"\n#################################################################\n");
        ...
        write(
"#################################################################\n"+
"Scrying Eye->There is no "+ str +" here.\n\n"+
"#################################################################\n");
      }
    }
    return;
...
status cast_spell(object caster,object target,object prev,int level) {
  ...
  if((ob = present("wizard eye", caster))) {
  ...
  eye_controller = clone_object("inherit/objs/scrying_obj");
