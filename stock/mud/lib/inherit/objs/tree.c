...
int counter, growing, number_branches,number_fruit;
...
void init() {
  ...
  add_action("take","get");
  this_object()->grow(); // this file is for trees, not seeds.
}
...
status take(string str) {
  ...
  if(sscanf(str, "%sbranch%s", tmp1, tmp2)) {
   ...
   branch = clone_object("inherit/objs/branch");
    wt = (int)branch->query_weight();
    if(!this_player()->add_weight(wt)) {
      write("The branch is too heavy for you.\n");
      destruct(branch);
...
status pick(string str){
  ...
  if(number_fruit < 1){
    write("You don't see any of those.\n");
    return 1;
  }
  number_fruit--;
