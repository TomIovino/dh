...
/* Default */
  ...
  set_alt_name("spell");
  set_alias("bottle");
  set_short("bottle of water");
  set_long("A Bottle Of Water.\n");
  set_room_msg("You see @@query_name:$this_player()$@@ slam down a bottle"+
               " of @@query_possessive:$this_player()$@@ water.\n");
  set_consumer_msg("You slam down a bottle of water.\n");
  set_strength(1);
  set_alt_name("spell");
  set_alias("spell");
}
...
status drink(string str) {
  ...
  if((((int)tp->query_true_stat("constitution")
           +tp->query_bonus_stat("constitution"))*6) < strength){
