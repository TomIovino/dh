...
//      tell_object(find_player("geewhiz"),sprintf("%-880s\n",m));
// }
...
void tell_channel(string chan, string name, string message, int no_history) {
  ...
  B_MAGENTA_F,str,chan,B_MAGENTA_F,NORM,880-cstrlen(chan),mesg);

  if(chan == "intermud" || chan == "interadmin" || chan == "intercode")
  mesg = sprintf("%s{%s%s%s}%s %-=*s",
  "",str,chan,"","",880-cstrlen(chan),mesg);

  if(chan == "divinity" ||
     chan == "mortal")
  mesg = sprintf("%s[*%s%s%s*]%s %-=*s",
  B_GREEN_F,str,chan,B_GREEN_F,NORM,880-cstrlen(chan),mesg);

  if(chan == "announce")
  mesg = sprintf("%s[%s%s%s]%s %-=*s",
  B_RED_F,str,chan,NORM+B_RED_F,NORM,880-cstrlen(chan),mesg);

  if(chan == "world")
  mesg = sprintf("%s[%s%s%s]%s %-=*s",
  BLUE_F,str,chan,NORM+BLUE_F,NORM,880-cstrlen(chan),mesg);

  if(chan == "info")
  mesg = sprintf("%s[%s%s%s]%s %-=*s",
  B_RED_F,str,NORM+chan+B_RED_F,NORM+B_RED_F,NORM,880-cstrlen(chan),mesg);

  if(chan == "monitor")
  mesg = sprintf("%s[%s%s%s]%s %-=*s",
  GREY_F,str,chan,NORM+GREY_F,NORM,880-cstrlen(chan),mesg);
  ...
  mesg = sprintf("%s[%s%s%s]%s %-=*s",
  YELLOW_F,str,chan,YELLOW_F,NORM,880-cstrlen(chan),mesg);
