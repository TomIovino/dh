...
string qsr(int me, int max){
    int per;
    string doh;
    per = (100 * me / (max));
    switch(per){
        case      0:  doh =  NORM       "void      ";       break;
        ...
        default:     doh =   NORM       "*ASHLIKE* ";       break;
     }
     return doh;
}


mixed is_valid_class(string class,string elem) {
    mixed ret;
    mapping sub;
    if(!class) return copy_mapping(valid_class);
    if(!elem) return copy_mapping(valid_class[class]);
    sub = valid_class[class];
    if(sub[elem]){
     ret = sub[elem];
     return ret;
    } 
...
string is_class(string subclass)
{     object pla;
      pla = this_player();
      if( is_valid_class(pla->query_profession(),pla->query_magic())
          == subclass)  
      {
             pla->set_class(subclass);
             return pla->query_class();
      }
      if( is_valid_class(pla->query_profession(),pla->query_faith())
          == subclass)  
      {
      ...
      pla->set_class(pla->query_profession());
    return pla->query_class();
}
