...
varargs void add_cmd_file(string dir, int min_level, int x);
varargs void fill_cmd_table(string dir, int min_level, int x);
varargs void rehash(int x);
...
varargs void add_cmd_file(string file, int min_level, int no_load) {
...
varargs void fill_cmd_table(string dir, int min_level, int no_load) {
    string *files, file; 
    int i;
    ...
    while(i--) 
        add_cmd_file(sprintf("/%s/%s",dir, files[i]), min_level, no_load);
}
 
 
varargs void rehash(int no_load_commands) {
    int i;
DBG("bin_d: rehash(" + no_load_commands + ");");
    cmd_table = ([ ]);
    log(C_INFO,"Command Table Rehashed");
    if(i = sizeof(list))
      while(i--) fill_cmd_table(list[i][DIR],list[i][LEV],no_load_commands);
}

int security(int min_security) {
    int security_lev;
    
    security_lev = (int) call_other(this_player(), LEVEL_FUNC);
    return funcall(LEVEL_TEST,security_lev, min_security);
}

int _do_rehash(closure func, string verb, string arg) {
    closure cmd_main, cmd_help;
    int min_sec_level;
    string file;
    
    if(mapping_contains(&cmd_main, &cmd_help,
       &min_sec_level, &file, cmd_table, verb)) {
        log(C_WARNING,sprintf("Auto Rehashed %O",file));
        add_cmd_file(file, min_sec_level);
        return funcall(func, verb, arg);
    }
    return 0;
}

varargs
int _do_cmd(string verb, string arg, int skip) {
    closure cmd_main, cmd_help;
    int min_sec_level;
    string file;
    ...
    if(mapping_contains(&cmd_main, &cmd_help,
       &min_sec_level, &file, cmd_table, verb)) {
        if(security(min_sec_level)) {
            if(!cmd_main)
                throw(OBJ_DEST_ERR);
            return funcall(cmd_main, verb, arg);
        }
    }
    return 0;
}
...
varargs
int _do_help(string verb, string null, int skip) {
    closure cmd_main, cmd_help;
    int min_sec_level;
    string file;
