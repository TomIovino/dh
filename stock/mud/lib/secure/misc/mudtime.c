...
#define TIME_OF_CREATION        (815162400) // 10 am, Oct 31,1995
#define FIRST_MUD_YEAR          (1)
#define AGE_OF_MUD              (time() - TIME_OF_CREATION)
#define MUD_YEAR                (900 * 24 * 7 * 52) //(60 * 60 * 24 * 7 * 15) //(7432520)
#define MUD_WEEK                (MUD_YEAR / 52)
#define MUD_MONTH               (MUD_YEAR / 12)
#define MUD_DAY                 (MUD_WEEK / 7)
#define MUD_HOUR                900 //(MUD_DAY / 24)
...
varargs int mud_month(int t) {
    return (mud_time_this_year(t) / MUD_MONTH) % 12;
}
...
//  printf("mud_minute() = %d  %% = %d  m + %% = %d\n",mud_minute(),15-mud_minute()%15,mud_minute()+(15-mud_minute()%15));

  diff = ((15 - mud_minute()%15) * MUD_MINUTE);
  if((MUD_QUARTER - diff) < 0)
    diff -= MUD_QUARTER;
//  printf("MUD_QUARTER - diff = %d\n",MUD_QUARTER - diff);
//  printf("mud_ctime() = %s\n",mud_ctime());
//  printf("mud_ctime(time()) = %s\n",mud_ctime(time()));
//  printf("time() - mud_time() = %d\n",time() - mud_time());
//  printf("Qtr. Boundry is %s (%d) away.\n",timestr(diff,1),diff);
//  printf("mud_ctime(mud_time() + boundry) = %s\n",mud_ctime(mud_time() + diff));
//  printf("%-=75s\n","It is " + query_mud_time() + ".");
//  call_out("test_time",diff,mud_time() + diff);
  call_out("update_time",0);

//  for(x = 0; x <= 1010800; x+=random(10000))
//   printf("x = %d | mct = %s\n",x,mud_ctime(x));
}

void test_time(int expected) {
  int diff;
...
void call_time_change(object ob,int qtr, int hr,int mn,int day,int month,int date,int year) {
    int ret;
    if(!ob)
        return;
    catch(ret = (int) ob->time_change(qtr,hr,mn,day,month,date,year));
    if(!ret)
        time_change_notices -= ({ ob });
}

void call_out_time_changes(object ob,int qtr, int hr,int mn,int day,int month,int date,int year) {
    call_out("call_time_change",0,ob,qtr,hr,mn,day,month,date,year);
}
...
void broadcast_time(int when, int data) {
    ...
    if(when < 0)
        return;
    time_change_notices[when] -= ({ 0 });
    map_array(time_change_notices[when],#'call_out_time_changes,q,h,m,dy,mn,dt,yr);
}    
...
*/
    ...
    if(day_night != last_day_night) {
        ...
        switch(day_night) {   
            case 0:
                if(quarter == 0 && hour == 5)
                    tell_everyone(B_MAGENTA_F "The sky turns to Dawn." NORM "\n");
 //   tell_everyone(B_GREEN_F + sprintf("%-=65s","It is " + query_mud_time() + ".") + NORM "\n");
                day_status = light_adj = 0;
                break;
            case 1: 
                if(quarter == 0 && hour == 6)
            ...
            case 2:
                if(quarter == 0 && hour == 17)
                    tell_everyone(B_BLUE_F "The sky turns to Dusk." NORM "\n");
                day_status = light_adj = 0;
                break;
            case 3:
