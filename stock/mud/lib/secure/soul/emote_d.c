...
int dump_stats(string arg) {
   ...
   if(!arg || arg == "") {
       more(sprintf("%|79s\n%-#79s\n",
          ...
          implode(data,"\n")),sprintf("Feelings Statistics (%d Emotes)",sizeof(data)));
   } else {
      i = 10;
...
int dump_stats_by_sex(string arg) {
   ...
   more(sprintf("%|79s\n%|79s\n%-#79s\n",
        ...
        "male/female/unknown",
        implode(data,"\n")),sprintf("Feelings Statistics By Sex (%d Emotes)",sizeof(data)));
    return 1;
}
...
status dump_feelings(string arg) {
    ...
    if(!short)
        more(sprintf("%-#80s\n",implode(tmp,"\n")),
            sprintf("Feelings (%d Emotes)",sizeof(tmp)));
    else
        more(sprintf("%-=78s\n",implode(tmp,", ")),
            sprintf("Feelings (%d Emotes)",sizeof(tmp)));
...
void page(string data,string header) {
    ...
    d = explode(data,"\n");
        ...
        (void)pager->more_text(d, header ? header : "Feelings");
}
