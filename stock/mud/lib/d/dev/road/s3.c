#define AWAY "south"
#define TOWARDS "north"
#define ROAD_NAME "Southern Road"
#include "../element.h"
#include <ansi.h>
#include <mudlib.h>
inherit ROOM;

void reset(status arg) {
   if(arg) return;
    set_type(ELEMENT);
    set_short(ROAD_NAME);
    set_long(COLOR+
"North leads slightly uphill to the main square of "+TOWN+".\n"+
"Directly south are the protective "+TOWN+" town gates.\n"+
"West is the entrance for the "+BANK+".\n"+
"A sign for "+PUB+" is north of here.\n"+
    NORM);
    set_items(({
    "u#up#sky",COLOR+UP+".\n",
    "d#down#street#ground#road",COLOR+DOWN+".\n",
    "walls#gates#citywalls",COLOR+WALLS+".\n",
   }));
   set_smell(SMELL"\n");
   set_listen(SOUND"\n");
   set_light(5);
   set_exits(({
          PATH+"road/sgates",AWAY,
	  PATH+"road/s2",TOWARDS,
          PATH+"room/bank","west",
   }));

}


