...
void reset(status arg) {
   if(arg) return;
    ...
    set_long(BROWN_F+
"These are the steps in front of the northeast tower of Twilite. A metal\n"+
...
"Steps lead down from here to the path that circles the city proper.\n"+
   NORM);
    ...
    "door", B_GREEN_F+
    "Examining the door you recognize the elemental symbol of Earth.\n",
   ...
   set_light(5);
   set_exits(({
	"d/town/road/toe1","down",
   }));
   set_door_path("/d/town/tower/toe3");
   set_door_name("door");
