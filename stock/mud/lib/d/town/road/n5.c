...
void reset(status arg) {
   if(arg) return;
    set_short(ROAD_NAME);
    set_long(BROWN_F+
"Directly to the north the city gates can be seen leading into the great\n"+
"forest beyond. Southwest of here an entrance leads to the castle\n"+
...
"district from the rest of town.\n"+
   ...
   set_light(5);
   set_exits(({
	"d/town/road/ngates","north",
	"d/town/road/n4",TOWARDS,
   }));
