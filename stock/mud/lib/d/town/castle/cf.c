...
void reset(status arg) {
  add_object("elite","d/town/npc/elite");
    if(arg) return;
    set_short("North Castle Chamber");
    set_long(GREY_F+
"This chamber is well lit to allow activities such as reading and\n"+
"sparring. South leads back to the castle's entrance while halls\n"+
"lead west and east to other chambers.\n"+
  ...
  "Well worn cobblestones worked together to form the road.\n",
    ...
    set_exits(({
	"d/town/castle/cc","south",
	"d/town/castle/cg","west",
	"d/town/castle/ce","east",
      }));
   set_light(5);
}
