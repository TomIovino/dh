...
status main(string targ, mixed alt_type, mixed spell_type){
 ...
 dam = ( random(stat+skill+level)+stat+skill+level ) / 4;
	...
	"level",  35,
	"cast time", 2,
	"msg target", COLOR+desc+" "+ELEMENT+" bolt springs from "+NAME
	+ "'s hand, striking you!\n"+NORM,
	"msg room", COLOR+desc+" "+ELEMENT+" bolt springs from "+ NAME
	+ "'s hand, striking "  +TARG+ "!\n"+NORM,
	"msg caster", COLOR+desc+" "+ELEMENT+" bolt springs from your hand " 
	+ "striking "+ TARG + NORM +"!\n",
      }));
    return 1;
}
