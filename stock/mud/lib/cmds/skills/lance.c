...
status main(string targ, mixed alt_type, mixed spell_type){
    ...
    desc = ""; 
    mod = (stat+skill)/50;
    dam = ( random(stat+skill+level)+stat+skill+level ) / 2;
    dam += level;
	...
	"level",  55,
	"cast time", 2,
	"msg target", COLOR+desc+" "+ELEMENT+" lance springs from "+NAME
	+ "'s hand, striking you!\n"+NORM,
	"msg room", COLOR+desc+" "+ELEMENT+" lance springs from "+ NAME
	+ "'s hand, striking "  +TARG+ "!\n"+NORM,
	"msg caster", COLOR+desc+" "+ELEMENT+" lance springs from your hand " 
	+ "striking "+ TARG + NORM +"!\n",
      }));
    return 1;
}
