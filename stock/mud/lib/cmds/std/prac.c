...
string *query_cmd_aliases() { return ({"disc","disciplines","practice" });}
...
//   printf((count % 2 ? "\n" : ""));
   write("\n-*Total Disciplines: "+TP->query_total_skills());
   write("*-*Total Notches: "+t+"*-*Average Discipline: "
      +(t/TP->query_total_skills())+"*-\n");
   printf("%s\n",PRINTBAR);
   return 1;
}
