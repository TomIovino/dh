...
reset (status arg){
        ...
        set_name("sutcher");
        set_alias("dagger");
  set_short(CO"red dagger with a "+
  ...
  WHITE_F"e"CYAN_F"d " +NORM+CO+"hilt"+NORM);
     set_wield_str(CO"You feel the power of dark"+BOLD+
                     " flame flowing through you.\n");
  set_info(CO+"This dagger is known as 'SUTCHER'.\n"+NORM+
...
int weapon_hit(object attacker) {
        ...
        r = random(100);
        if(r < 10){
  write(B_RED_F+"Sutcher sinks into "+attacker->QN+".\n"NORM);
  say(B_RED_F+"Sutcher sinks into "+attacker->QN+".\n"NORM);
  if(attacker->query_level() > 25){
    write(B_RED_F+"You feel energy sift into you from Sutcher!\n"NORM);
    attacker->adj_hp(-5);
