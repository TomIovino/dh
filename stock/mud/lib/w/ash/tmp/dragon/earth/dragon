...
#define PATH   "/w/ash/dragon/greysteel/"
...
reset (arg){
   ...
   if(arg) return;
   set_name("crawler");
   set_short(GREY_F+"Ancient Earth Dragon");
   set_dead_ob(this_object());
   set_long(B_RED_F+
   "The words of a poem about death race through your mind.\n"+
   "A spectrum of grey scales shine brightly on the ancient\n"+
   "skin of Crawler, You feel sick with fear.\n"NORM);
  set_race("dragon");
  set_aggressive(1);
  set_gender(1);
  load_spells(10, ({
  ...
  load_a_chat(10, ({ 
    B_RED_F+"Crawler whips his tail around nearly hitting you!\n" }));
   set_level(150);
   add_object(CLONE[c],PATH+CLONE[c]);
   add_object("hammer",PATH+"artifact");
    ...
    if(random(100) <= 10) {
        ...
        for(i = 0; i < sizeof(a); i++) {
            ...
            a[i]!=this_object()) {
                ...
                a[i]->adj_hp(dam);
           tell_object(a[i],
    YELLOW_F"Crawler breaths POISON dust breath all over you!\n"+NORM);
                if(a[i]->query_hp()==0){
                   a[i]->death();
                }
                 primary_attack = a[i];
