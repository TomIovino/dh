...
#define WIZARD_SAVE_DIR "usr/creators/"
#define NO_BANISH       "usr/no_banish/"
...
varargs void move_player(string dir_dest, mixed optional_dest_ob) {  
  ...
  if(environment()) tell_room(environment(), capitalize(name)+" leaves.\n");
  move_object(this_object(), dest);  
  if(environment()) tell_room(environment(), capitalize(name)+" arrives.\n");
  ob = environment();  
  ob->long(1);
...
varargs string query_name(status arg) { 
 if(!name || !stringp(name)) name = "debuglogon";
...
static void logon2(string str) { 
  ...
  str = lower_case(str);  
  name = str;
  if(!restore_object(WIZARD_SAVE_DIR + name)) {
    restore_object(NO_BANISH + name);
  }
  if(!secure(SEC9)) {  
    write("\n\nSorry. Dragonheart is closed to Mortals right now.\n");
    remove_call_out("time_out"); 
    destruct(this_object());  
    return;
  }
...
static void logon3() { 
  ...
  write("!! PLAYER OBJECTS ARE CORRUPTED !!\n"+
        "\n"+
        "Geewhiz please read the dragon account mail =) (Ash)\n"+
        "\n"+
  ...
  if(catch(move_player("X#"+ INNER))) move_player("X#room/void");
}  
...
static status update(string str) {  
  ...
  destruct(ob); /* note that master.c takes care of player of room dested */  
  return 1;  
}  
