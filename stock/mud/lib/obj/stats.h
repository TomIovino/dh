...
int shouldpray(){
       int should,current,bonus,x;
       x = this_object()->query_level();
       current = this_object()->query_stats_total();
       should  = 200;

       if(x == 100) should = 600;
       else if(x >= 75) should += (350 + ((x-75)*2));  
       else if(x >= 50) should += (275 + ((x-50)*3));  
       else if(x >= 25) should += (150 + ((x-25)*5));
       else should += x*6;
...
void check_extra_stats(){
      ...
      write("opb             "+query_extra_stat("opb")+"/2\t\t");
      write("dpb             "+query_extra_stat("dpb")+"/2\n");
      write("spb             "+query_extra_stat("spb")+"/2\t\t");
      write("cpb             "+query_extra_stat("cpb")+"/2\n");
}
...
varargs mixed query_stat(string var) {
    ...
    if(ret > 100+query_extra_stat(var)) ret = 100+query_extra_stat(var);
    return ret;
}
...
int init_stats(string rstr,string pstr){
   ...
   if(rstr == "dragonkin"){
   this_object()->set_alignment("cosmos");
   this_object()->set_magic("elemental");
   this_object()->set_faith("divinity");
   set_all_maxstats(80);
   }
   if(rstr == "gremlin"){
   set_alignment("gehen");
   ...
   if(pstr == "warrior") { adj_max_stat("strength",5);  }
   if(pstr == "warrior") { adj_max_stat("stamina",5);    }
   if(pstr == "rogue")   { adj_max_stat("agility",5);    }
   if(pstr == "rogue")   { adj_max_stat("dexterity",5); }
   if(pstr == "cleric")  { adj_max_stat("wisdom",5);    }
   if(pstr == "cleric")  { adj_max_stat("focus",5);      }
   if(pstr == "magician"){ adj_max_stat("intellect",5);  }
   if(pstr == "magician"){ adj_max_stat("focus",5);     }
